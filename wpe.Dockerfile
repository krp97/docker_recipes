...
RUN apt-get -qq update --yes && apt-get -qq upgrade --yes 
RUN apt-get -qq install software-properties-common build-essential wget python3 python3-pip \
	zlib1g-dev libssl-dev git psmisc vim curl iproute2 gdb htop --yes --fix-missing
...
RUN cmake .. -DCMAKE_INSTALL_PREFIX=/usr \
        ...
        -DPLUGIN_NETWORKCONTROL_INTERFACES="eth0:Dynamic:-:-:-" \
		...
		-DPLUGIN_WEBSERVER_PORT=8080 \
		-DPLUGIN_WEBSERVER_PATH="/www" \
...
WORKDIR /root/repos/ThunderUI
RUN mkdir -p /usr/share/WPEFramework/Controller/UI && \
	cp -r dist/* /usr/share/WPEFramework/Controller/UI/
